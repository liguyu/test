...
import com.aofeng.wellbeing.model.addressRowModel;
...
//
	...
	// 地址列表
	public ArrayListObservable<addressRowModel> addressList = new ArrayListObservable<addressRowModel>(
			addressRowModel.class);
	//统计信息
	...
	public void ShowList(){
		...
		Cursor c = db.rawQuery("select DISTRICTNAME, ADDRESS from T_IC_SAFECHECK_PAPAER order by address", null);
		while(c.moveToNext()) {
			String districtname = c.getString(c.getColumnIndex("DISTRICTNAME"));
			String address = c.getString(c.getColumnIndex("ADDRESS"));
			
			addressRowModel addressModel = new addressRowModel(mContext);
			addressModel.model = BigAddressModel.this;
			addressModel.districtname.set(districtname);
			addressModel.address.set(address);
