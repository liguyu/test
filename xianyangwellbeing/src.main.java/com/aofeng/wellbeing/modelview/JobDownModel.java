...
import com.aofeng.wellbeing.activity.JobDownActivity;
import com.aofeng.wellbeing.model.JobRowModel;
...
public class JobDownModel {
	...
	public StringObservable username = new StringObservable("系统管理员");
	public StringObservable date = new StringObservable("2014-12-11 00:00:00");
			...
			public void run() {
				try {
					String url = Vault.DB_URL
							+ URLEncoder.encode("from T_CHECKPLAN t where t.f_issued='是' and t.f_checkman = '系统管理员' and t.f_date <= '" + date.get() + "' and t.f_enddate >= '" + date.get() + "' order by f_date desc", "UTF8")
									.replace("+", "%20");
